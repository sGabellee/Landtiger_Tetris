...
void RIT_IRQHandler (void)
{
 ...
 // --- MODIFICA 2: ADC Non-Blocking (Pipeline) ---
 // A. Leggi il risultato chiesto 50ms fa (se pronto)
 if(((LPC_ADC_TypeDef *) ((0x40000000UL) + 0x34000) )->ADDR5 & 0x80000000){
  AD_current = ((((LPC_ADC_TypeDef *) ((0x40000000UL) + 0x34000) )->ADDR5 >> 4) & 0xFFF);
 }
 // B. Lancia la conversione per il PROSSIMO interrupt (senza aspettare!)
 ...
 if (slow_mode_counter > 0) {
  target_MR0 = 25000000; // Forza velocità Lenta (1 sec)
  slow_mode_counter--; // Decrementa il contatore (tick da 50ms)
 }

 // 3. Joystick DOWN (Soft Drop) -> Raddoppia velocità
 if(gameState == 1 && (((LPC_GPIO_TypeDef *) ((0x2009C000UL) + 0x00020) )->FIOPIN & (1<<26)) == 0){
  target_MR0 = target_MR0 / 2;
  if(((LPC_TIM_TypeDef *) ((0x40000000UL) + 0x04000) )->TC > target_MR0/2) {
   ((LPC_TIM_TypeDef *) ((0x40000000UL) + 0x04000) )->TC = 0;
  }
 }

 // 4. Smart Update del Timer
 if(gameState == 1 && ((LPC_TIM_TypeDef *) ((0x40000000UL) + 0x04000) )->MR0 != target_MR0) {
  ((LPC_TIM_TypeDef *) ((0x40000000UL) + 0x04000) )->MR0 = target_MR0;
  if(((LPC_TIM_TypeDef *) ((0x40000000UL) + 0x04000) )->TC > target_MR0) {
   ((LPC_TIM_TypeDef *) ((0x40000000UL) + 0x04000) )->TC = 0;
  }
 }
  ...
  switch(downKey1){
   case 1:
    ...
    // PLAY -> PAUSA
     ...
     gameState = 0;
     disable_timer(0);
     GUI_Text(100, 150, (uint8_t *)"PAUSED", 0xF800, 0x0000);
    }
    // GAMEOVER -> RESTART
    else if (gameState == 2) {
     ...
     LCD_Clear(0x0000);
     // ridisegna bordi
     LCD_DrawLine(5 - 1, 10 - 1, 5 - 1, 10 + (20 * 15) + 1, 0xFFFF);
     LCD_DrawLine(5 + (10 * 15) + 1, 10 - 1, 5 + (10 * 15) + 1, 10 + (20 * 15) + 1, 0xFFFF);
     LCD_DrawLine(5 - 1, 10 + (20 * 15) + 1, 5 + (10 * 15) + 1, 10 + (20 * 15) + 1, 0xFFFF);
     GUI_Text(165, 40, (uint8_t *) "TOP", 0xFFFF, 0x0000);
     GUI_Text(165, 100, (uint8_t *) "SCORE", 0xFFFF, 0x0000);
     GUI_Text(165, 160, (uint8_t *) "LINES", 0xFFFF, 0x0000);
     updateStats();
     spawnNewPiece();
     // primo pezzo
     drawTetromino(curr_x, curr_y, currPieceType, currRot, 1);
